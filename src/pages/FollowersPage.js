...
export default function FollowersPage({ updateApp }) {
    ...
    useEffect(() => {
        ...
        axios.get(`${process.env.REACT_APP_API_URL}/followers/${token.userId}`, {
        ...
        for (let index = 0; index < users.length; index++) {
            axios.get(`${process.env.REACT_APP_API_URL}/user/${users[index]}`, {
                headers: { Authorization: `Bearer ${token.token}` }
            })
                .then(res => setUserData([...usersData, res.data]))
                .catch(err => {
                    if (Array.isArray(err.response.data)) {
                        err.response.data.forEach(element => {
                            alert(element);
